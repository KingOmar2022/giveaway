...
async def on_ready():
    ...
    servers = len(client.guilds)
    await client.change_presence(status=discord.Status.dnd, activity = discord.Game(name=f'{servers} servers {members} people'))
...
async def help(ctx):
    ...
    mbed.set_author(name = 'Help', icon_url = '')
    mbed.add_field(name=f'{prefix}help', value = 'Shows list of help commands', inline = False)
    mbed.add_field(name=f'{prefix}giveaway', value = 'can be access with manage messages perms', inline = False)
    mbed.add_field(name=f'{prefix}reroll `<channel_name>` `[message_id]`', value = 'can be access with manage messages perms', inline = False)
    mbed.set_footer(text=f'more commands coming soon [BETA VERSION]')
...
# Bot token
client.run("OTg3ODEyMjY0NDI4MDQ4NDM1.GuUh_r.mAGtGAcfxmOv7NLoJaSNgeat2gWVfQ-WXRhfJk")